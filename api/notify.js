  ...
  return {
    ...
    user:
      ...
      `前7天未完成（todo）任务：\n${payload.past7
        .map(
          (d) =>
            `- ${d.date}${d.holiday ? `(${d.holiday})` : ""}: ${
              d.todo.length ? d.todo.join("；") : "无"
            }`,
        )
        .join("\n")}\n\n` +
      `今日全部任务：\n` +
      `- todo: ${payload.today.tasks.todo.length ? payload.today.tasks.todo.join("；") : "无"}\n` +
      `- done: ${payload.today.tasks.done.length ? payload.today.tasks.done.join("；") : "无"}\n` +
      `- giveup: ${payload.today.tasks.giveup.length ? payload.today.tasks.giveup.join("；") : "无"}\n\n` +
      `后3天任务（todo）：\n${payload.next3
        .map(
          (d) =>
            `- ${d.date}${d.holiday ? `(${d.holiday})` : ""}: ${
